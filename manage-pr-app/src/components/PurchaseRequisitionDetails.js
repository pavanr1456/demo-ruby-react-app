...
const GET_ITEMS_API_URL = (id) =>
  `${BASE_URL}/api/v1/purchase_requisitions/${id}/items`;
const GET_ITEM_UPDATE_URL = (id, itemId) =>
  `${BASE_URL}/api/v1/purchase_requisitions/${id}/items/${itemId}`;
const GET_ITEM_CREATE_URL = (id) =>
  `${BASE_URL}/api/v1/purchase_requisitions/${id}/items`;
...
function PurchaseRequisitionDetails() {
    ...
    axios
      ...
      .catch((error) => {
        if (error.response && error.response.data) {
          ...
          setErrors({
            pr_type:
              errorData.pr_type?.length > 0 ? errorData.pr_type[0] : null,
            description:
              errorData.description?.length > 0
  ...
  const handleUpdateItem = () => {
    const { item_name, quantity, unit_price, total_price, notes } = items.find(
      (item) => item.id === currentItemKey
    );
    ...
    if (currentItemKey === "100") {
      ...
      axios
        ...
        .then((response) => {
          ...
          // Fetch updated items after successful creation
          return axios.get(GET_ITEMS_API_URL(requisition.id));
        })
        .then((res) => {
          setItems(res.data); // Update items in state
        })
        .catch((error) => {
          if (error.response && error.response.data) {
            const errorData = error.response.data;
            const itemLevelError = {};
            itemLevelError[`item_name[${currentItemKey}]`] =
              errorData.item_name?.length > 0 ? errorData.item_name[0] : null;
  ...
  const hanldeAddItem = () => {
    setItems([
      ...items,
      { id: "100", item_name: "", quantity: 1, unit_price: 0, notes: "" },
    ]);
  ...
  const handleItemSelection = (oEvent) => {
    if (Number(oEvent.target.selected) !== currentItemKey) {
      const selectedItem = items.find(
        (item) => item.id === Number(oEvent.target.selected)
      );
  ...
  return (
    <ObjectPage
      ...
      titleArea={
        <ObjectPageTitle
          ...
          header={`Purchase Requisition ${requisition.id}`}
    ...
    >
        ...
        <ObjectPageSubSection
          ...
          actions={
            <div style={{ marginLeft: "auto", display: "flex" }}>
              {isEditMode ? (
                <Button design="Emphasized" onClick={handleUpdate}>
                  Save
                </Button>
              ) : null}
            ...
            <FormItem labelContent={<Label showColon>Type</Label>}>
              {isEditMode ? (
                <Select
                  value={pr_type}
                  valueState={
                    errors.pr_type && errors.pr_type != null
                      ? "Negative"
                      : "None"
                  }
                ...
                >
                  ...
                  <Option value="NBS">
                    {" "}
                    NBS (Standard Purchase Requisition){" "}
                  </Option>
                ...
                <Text>{requisition.pr_type_desc}</Text>
              )}
            </FormItem>
      ...
      <ObjectPageSection aria-label="Items" id="items" titleText="Items">
        <ObjectPageSubSection
          titleText=""
          actions={
            <div style={{ marginLeft: "auto", display: "flex" }}>
              <Button
                ...
                onClick={hanldeAddItem}
              >
                Add Item
              </Button>
              {currentItemKey && !isItemEditMode ? (
                <Button design="Transparent" onClick={handleToggleItemEditMode}>
                  Edit
                </Button>
              ) : null}

              {currentItemKey && isItemEditMode ? (
                <Button design="Transparent" onClick={handleUpdateItem}>
                  Save
                </Button>
              ) : null}

              {currentItemKey && isItemEditMode ? (
                <Button design="Transparent" onClick={handleCancelItem}>
                  Cancel
                </Button>
              ) : null}
            </div>
          ...
          <Table
            ...
            features={
              <TableSelection
                ...
                selected={
                  typeof currentItemKey === "string" ||
                  currentItemKey instanceof String
                    ? currentItemKey
                    : " "
                }
              />
          ...
          >
            {items.map((row, index) => (
              <TableRow rowKey={row.id}>
                <TableCell>
                  {currentItemKey === row.id && isItemEditMode ? (
                    <Input
                      name="item_name"
                      value={row.item_name}
                      onChange={(event) => handleItemChange(index, event)}
                      valueState={
                        itemErrors.hasOwnProperty(`item_name[${row.id}]`) &&
                        itemErrors[`item_name[${row.id}]`] != null
                          ? "Negative"
                          : "None"
                      }
                    >
                      <div slot="valueStateMessage">
                        {itemErrors[`item_name[${row.id}]`]}
                      </div>
                    </Input>
                  ) : (
